...
Valor.prototype.getChampions = function(region, cb){
	...
	request('https://'+region+constants.champions+region+'/v1.2/champion' + '?api_key='+ this.api_key, function (error, response, body) {
...
Valor.prototype.getChampionById = function(id, region, cb){
		...
		request('https://'+region+constants.champions+region+'/v1.2/champion/'+ id +'?api_key='+ this.api_key,function(error,response,body) {
		if (!error && response.statusCode === 200) {
			cb(null,JSON.parse(body));
			}
		else{
...
Valor.prototype.getSummonerByName = function(name, region,cb){
	console.log('Valor..TO ME summoner by name: '.cyan, name.green  );
	request('https://'+region+constants.summoner +region+'/v1.4/summoner/'+'by-name/'+name+'?api_key='+this.api_key, function (error, response, body) {
		if(!error && response.statusCode === 200) {
...
Valor.prototype.getSummonerById = function(id, region,cb) {
	console.log('Valor..TO ME summoner by ID: '.cyan, id.toString().green  );
	request('https://'+region+constants.summoner +region+'/v1.4/summoner/'
							+ id + '?api_key=' + this.api_key, function (error, response, body) {
		if (!error && response.statusCode === 200) {
			//console.log(body);
			cb(null,JSON.parse(body));
		}
